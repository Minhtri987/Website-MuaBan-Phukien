...
class ProductController
{
    ...
    public function delete($id)
    {
        ...
        $product = $this->productModel->getProductById($id);
        if (empty($product)) {
            // Product not found, redirect to error page or display error message
            include_once 'app/views/share/not-found.php';
        ...
        if ($result) {
            // Product deleted successfully, redirect to product list page
            echo "<script>alert('Product deleted successfully !')</script>";
            header('Location: /chieu21');
        } else {
            // Failed to delete product, display error message
